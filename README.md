...
前奏が長い音源には2通りの合わせ方があります。「**「音源の開始」まで待つ**」で切り替えます。

| | 音源 | 打ち込み |
| --- | --- | --- |
| OFF（既定） | 「音源の開始」から鳴る（前奏は飛ばす） | 曲頭から同時に始まる |
| ON | **頭から**鳴る（前奏も聴こえる） | 「音源の開始」まで**待ってから**始まる |

ONにしても**音符は1つも動きません**。後ろへずれるのは再生の開始時刻だけ（`sequencer.start` の
pre-roll）なので、曲データはそのままで、いつでもOFFに戻せます。曲の途中から再生したときは
待ちません（その位置の音源がすぐ鳴ります）。
...
| `#audiointro=on` | 前奏を鳴らしてから曲を始める（省略時オフ） |
