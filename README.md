...
グラフ外で鳴る後者2つは、**鳴り始めの遅れを実測して1回で詰め**（測った遅れぶん先へ seek する）、
以後は再生速度を±2%の範囲で微調整して寄せます（`preservesPitch` が効くので音程は動きません）。
大きくズレたときだけ seek で直します。

### 開始位置（いらないパートを飛ばす）

- **音源の開始** … 音源のどこから鳴らすか。`0:12.500` のように分:秒.ミリ秒、`12.5` のように秒だけでも可。
- **終了** … 音源のどこで止めるか。空欄なら最後まで。
- **曲の開始** … 上の「音源の開始」を曲のどこに合わせるか。**小節・拍と時間のどちらでも**指定でき、
  両方が連動します（時間で入れるとステップ単位まで置けるので、拍に乗らない位置へも合わせられます）。

再生中に動かしても、その場で合わせ直します（耳で追い込めます）。

### イントロが長い音源に合わせる

前奏が長い音源では、**イントロを飛ばして頭出しする**か、**イントロも鳴らして歌の開始を遅らせる**かの
2通りがあります。前者は「音源の開始」に前奏の長さを入れるだけですが、後者は打ち込み側を後ろへ
置く必要があり、「何小節目から書けばいいのか」を自分で計算する羽目になります。

そこで「**イントロも鳴らす**」ボタンを置いてあります。押すと

1. 打ち込み全体を「音源の開始」のぶんだけ後ろへずらし、
2. 「音源の開始」を `0:00.000` に戻します。

音源と打ち込みの相対関係は変わらないので、**聞こえ方（音源のどこで歌が入るか）はそのまま**、
前奏だけが鳴るようになります。歌の開始を遅らせる専用の設定は増やしていません——ピアノロール上でも
音符が実際にその位置へ動くので、見た目と聞こえ方が一致します。戻したいときは Undo。
...
| `#audioat=<ステップ>` | 音源を貼り付ける曲側の位置（1小節=192。省略時0） |
...
```
#audio=https://example.com/karaoke.mp3 #audiostart=12.5 #audioat=768 #audiovol=60;
@0 t120 o4 c d e f;
```
