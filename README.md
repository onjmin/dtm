...
音源側を後から引きずって直すと頭が飛ぶので、**逆向きに解決**しています。

1. 先に音源を鳴らし、**読み取り値が動いた瞬間**を捕まえて「いつ・どこを鳴らしていたか」を実測する
   （値が変わった瞬間なら、その値がたった今のものだと分かるので、更新が粗いYouTubeでも
   誤差をポーリング間隔まで押し込める）。
2. その実測から、**音源が同期位置へ達する時刻**を計算し、そこへ打ち込みの開始を合わせる
   （`sequencer.start` の pre-roll）。鳴り出しが何秒遅れても、音源と打ち込みの対応は変わりません。
3. 待つ余地が無かった（もう通り過ぎていた）ときだけ、音源側を実測で詰め直します。
   seek のたびに立ち上がりの遅れが乗るので、測って撃つのを最大3回繰り返して乗せます。
4. 以後は再生速度を±2%の範囲で微調整して追従（`preservesPitch` が効くので音程は動きません）。
   大きくズレたときだけ seek で直します。

待っている間は「音源が鳴り始めるのを待っています…」と表示します（5秒で諦めて通常再生へ）。
デコード済み（ファイル / CORS可のURL）は予約がサンプル単位で正確なので、この待ち合わせは起きません。

実測値: `<audio>` 直接再生で、再生開始0.4秒後から**±12ms以内**（従来は+104msから約5秒かけて収束）。
