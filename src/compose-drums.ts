/**
 * 自動作曲のドラム編曲。セクションの強度に合わせて曲全体のドラムを組み立てる。
 *
 * 参考にした人間の曲は**全曲がドラムに2〜31種（平均5.9種）の小節パターン**を持っていた。
 * イントロ、サビ頭のクラッシュ、4小節ごとのフィル、セクションごとのハイハットの開け閉め
 * ——曲の「進行している感じ」の大半はここが作っている。同じ1小節を鳴らし続けると、
 * 上物が何をしていても打ち込みらしさが残る。
 *
 * セクションの強度（0=抑えめ／1=標準／2=サビ）でキック・スネア・ハイハットの刻みを選び、
 * 4小節ごとの区切りにフィルを置く。フィルは「次の小節へ入る助走」なので、次が無い最終
 * 小節には置かず、**1つ手前**に置いてから最後はクラッシュとキックで鳴らし切る。
 *
 * `shuffle` だけは**16分の格子から外れる**。1小節192ステップは3で割り切れるので8分三連
 * （16ステップ）でハネを表現できる。参考曲は平均24%の音が16分格子の外にあり、生成物は
 * 0%だった——グルーヴを作るには格子から出る必要がある。
 */
