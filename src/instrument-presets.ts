...
export type InstrumentPreset = {
	 ...
	 *
	 * 上級者モードは伴奏を複数本に書き分けていて、奏法（`ChordPatternType`）は実際に別のリズムに
	 * なっている。ここが `chord` の使い回しだと、せっかく書き分けた層が同じ音色で重なって団子に
	 * なる。`chord` と喧嘩しない、かつ他のスロットと被らないものを選ぶこと。
	 */
	chordAlt: string;
	/**
	 * ウワモノ（装飾）の楽器。高い位置で薄く鳴らす、きらびやかな音。
	 * `chord` と喧嘩しない、他のスロットと被らないものを選ぶこと。
	 */
	sparkle: string;
	/**
