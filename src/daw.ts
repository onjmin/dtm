...
<div class="dtm-modal-body-content">
  ...
  <h4>歌入り作曲のボーカル</h4>
  <p>「歌入り作曲」は、作った主旋律に歌詞を付けて歌わせます。歌詞に意味はありません（文脈も情景も無いところから意味の通る詞は出せないので、<strong>メロディに正しく乗る、日本語として発音できる音の並び</strong>だけを作っています）。</p>
  <p><strong>上級者モード（15トラック）では、さらにボーカルアレンジまで展開します。</strong>どこで何をするかは曲ごとに自動で決まるので、設定項目はありません。</p>
  <ul>
    <li><strong>ハモリ</strong> — サビから、曲によってはBメロから入ります。<strong>主旋律と同じ声・同じ歌詞で、違う高さを重ねます</strong>。同じ歌い手が自分の声を重ねるので、そのパートから急に厚くなって豪華に聞こえます（別人の声を当てるとデュエットになってしまい、狙いが変わります）。音程は3度か6度が基本で、完全5度は浮くので避けます。主旋律がテンション音（9th・11th など）に乗っているところだけは4度で当てます——そこでは3度も6度も和音の構成音へ届かないためです。落ちサビはハモリを外します（伴奏を薄くして声だけを聞かせるのが「落ち」の実体なので）。</li>
    <li><strong>掛け合い（デュエット）</strong> — 4割ほどの曲で2人歌いになります。セクションごとの交代、Aメロで2小節ごとの交代、掛け合いサビ、Aメロだけ交代、の4通りから引きます。<strong>受け渡しは小節線ぴったりではなく、次の人が小節線の手前から食い気味に入ってきます</strong>。歌詞は主旋律ぶんをまとめて作ってから担当を割り振るので、交代の前後でも言葉が繋がります。一緒に歌うサビでは、相方が同じ音をなぞる（ユニゾン）のではなくハモリへ回ります。</li>
  </ul>
  <p>シンプルモード（4トラック）は独唱です。メロディ・サブメロ・ベース・伴奏で埋まっていて、ハモリを置くと伴奏かベースを潰すことになるためです。ボーカルアレンジまで欲しいときは上級者モードへ切り替えてください。</p>
  <h4>そのほか</h4>
  <ul>
		...
		const runCompose = (withVocal: boolean): void => {
			...
			overlayDuring(() => {
				...
				if (isAdvanced) {
					...
					// --- シンプルモード（4トラック）---
					// **`song.harmony` はここでは使わない。** 4本がメロディ・サブメロ・
					// ベース・伴奏で埋まっていて、ハモリを置くとどれかを潰すことになる。
					// ボーカルアレンジ（ハモリ・掛け合い）は advanced 限定という判断で、
					// シンプルモードは独唱のまま（`COMPOSE_INFO_HTML` に明記してある）。
