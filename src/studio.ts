...
export type DtmStudio = {
	 ...
	 * `position()` と `morae` を比べて進める。合成の遅い音源で行の途中に間が空くなら
	 * `minBufferSec`（0.3〜0.5 秒ほど）を足す。
	 *
	 * **既定（`awaitRender` 省略＝`false`）は頭が欠ける。** 計画が出来しだい時刻を決めるので速いが、
	 * 最初のチャンクの合成が間に合わなかったぶんを飛ばして途中から鳴らす（実測で速い音源でも
	 * 0〜450ms、遅い音源では 3.3 秒の行の 2.3 秒が欠けた）。既定は互換のため変えていない。
	 */
	speak: (
		text: string,
