/**
 * 楽器の「出口」を作る音作りブロック — ベース用のパラレル・サチュレーションと、ギター用の
 * キャビネットシミュレータ。どちらもトラックのチャンネルストリップの先頭（EQより前）へ
 * 差し込んで使う。
 *
 * サンプルを鳴らすだけの音源は「マイクで録った素の音」に相当する。実機のベースやギターは、
 * そのあと必ずアンプ／キャビネット／コンプという**出口**を通ってから耳に届いている。
 *
 * ## ベースにサチュレーションが要る理由
 *
 * 実際の再生環境はスマホの内蔵スピーカーが主で、小型スピーカーは 150〜300Hz 以下をほとんど
 * 再生しない。ベースの音域（A1〜A2 = 55〜110Hz）は**基音がまるごと出ない**ので、EQで低域を
 * 持ち上げても何も聞こえない。基音の整数倍音が 200〜2000Hz に並べば脳は鳴っていない基音を
 * 補完する（missing fundamental）ので、**低域を歪ませた成分を原音へ薄く混ぜる**。原音と並列
 * （パラレル）にするのは、直列で歪ませると低域そのものが潰れて逆に痩せるため。
 *
 * ## キャビネットIRを手続き的に作る理由
 *
 * `reverb.ts` と同じ方針で、外部のIRファイルを持たずに合成する。スピーカーのインパルス応答は
 * 「直接音＋箱とコーンの共振がすぐ減衰する」形をしているので、減衰する正弦波（モード）の
 * 重ね合わせで十分それらしくなる。
 */
