 ...
 * 取り込まない。休符（`Lyric=R`）とブレス（`息R` `R吸` `@br1`）はノートを作らない
 * ——このアプリのブレス `、` はノートを消費しない記号なので、直前の音節へ畳む。
 * 逆に、1音節が複数モーラになる歌詞（ピンインの `xing` → シ＋ン）では**ノートを割って**
 * 増やす（{@link splitNoteSteps}）。どちらの場合も「ノート1つにつき音節1つ」は保たれる。
 */
