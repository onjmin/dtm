/**
 * 生成した曲の「良さ」を測るための指標のうち、**順序に依存するもの**。
 *
 * 分布の指標（音価のエントロピー・休符率・跳躍率・音域）は、16小節の順番をシャッフルして曲を
 * 破壊しても値が動かない。モチーフの配置・盛り上がりの位置・フレーズの呼応は、そこでは評価
 * 対象にならない。
 *
 ...
 * （{@link MetricNote.pitchSemi}）とステップで受ける。
 */
...
/**
 * 1小節ぶんの指紋。**音高そのものは持たない**のが要点。人間の曲で繰り返されているのはリズムと
 * 輪郭で、音高で比べると「同じ小節をコピペしただけ」に高得点を与えてしまう。
 */
type BarSignature = {
...
/**
 * コール&レスポンス。サブメロの**音の入り（オンセット）**のうち、メロディが休んでいる瞬間に
 * 入るものの割合。
 *
 * 「鳴っているステップ」で測ると伸ばしている音が一方的に不利になる——メロディの隙間で入って
 * そのまま伸びるパッドや保続音は対旋律として正しい書き方なのに、伸びた分だけ重なりとして
 * 数えられる。測りたいのは「サブメロはメロディが息継ぎしたときに喋るか」なので、**入りの
 * 瞬間だけ**を見る。
 *
 ...
 * メロディがどれくらい細かく動いているか。
 *
 * 音価エントロピーも種類数も**分布の散らばり**しか見ておらず、「4分中心に散らばった曲」と
 * 「16分中心に散らばった曲」を区別しない。休符率も自己相似も同じ。これが無いと、16分の曲を
 * 引いても採点で落ちる。メロディの速さは曲の性格そのものなので、採点項目として持つ。
 */
...
/**
 * **「壊れていないか」だけを見る当てはめ。** p05〜p95 の内側なら一律1点、その外側で 0 へ落ちる。
 *
 * {@link band} は p25〜p75 を満点にして離れるほど減点する＝「各指標の真ん中に居るほど良い曲」
 * と言っているのと同じで、目標帯が21項目を独立に採った周辺分布である以上、その真ん中は
 * **どの曲でもない平均的な一点**でしかない。周辺分布の傾斜がある限り、散らばる人間の曲は必ず
 * 生成物に負ける。
 *
 * そこで周辺分布の項目は「人間の範囲に居るか」の確認だけに格下げし、「人間が書いた曲の形を
 * しているか」の判断は {@link nearestProfileDistance}（項目の**組み合わせ**を見る）へ渡す。
 ...
 * ので、そのまま距離を取ると `maxLeap` の 1 と `restRatio` の 1 が同じ重みになってしまう。
 * 帯の幅で割ると「コーパスの散らばりを1とした距離」に揃う。外れ値が距離を支配しないよう、
 * 帯の外側は ±0.5 ぶんで頭打ちにする。
 */
