...
export const buildAdvancedLayers = (
	...
	//
	// **音色を交互にする。** 奏法（`ChordPatternType`）は層ごとに別のものを引いていて、リズムは
	// 書き分けられている。全部が `chord` の1楽器だと、書き分けた層が同じ音色で重なって団子に
	// なる。パッド(t6)=chord に対して交互に置き、隣り合う層が必ず違う音色になるようにする。
	...
	layers.push(
		...
		// t12。3声目のハモリ（主旋律を上下から挟む3声）。曲ごとに出るかが決まる（`song.vocal`）。
		//
		// 出ない曲では**パッド(t6)が鳴っていないセクション**を、別の音色（`harmonyAlt` ＝声もの・
		// 柔らかい持続音）で受け持つ。パッドが覆っていないセクションは常にあるので、ここは**いま何も
		// 鳴っていない場所に持続音を足す**ことになる——重ねでも写しでもなく、純粋な追加。
		...
		//
		// ただしこの層は曲ごとに出るかどうかが決まり、出ない曲ではトラックが1本まるごと遊ぶ。
		// オクターブ等価の写しに常設の価値は無いが、**空けておく価値はもっと無い**ので、層が無い曲
		// では「t1（サビ重ね）が担当しないセクションの主旋律を、別の楽器でなぞる」層に回す。t1 は
		// CHORUS/LOUD 側を持つので、こちらは静かな側——結果として**セクションの境目で主旋律に付く
		// 音色が入れ替わる**。1トラック1楽器の制約下で「パートごとに楽器が変わる」を作れるのは、
		// この書き分けだけ。
		...
		// t14。**間奏のソロ。** 音が入るのは間奏の小節だけなので、この1本だけを別の楽器にしても
		// 他のセクションの鳴りは変わらない。歌の音域をそのまま渡すと管楽器が上へ抜ける
		// （テナーサックスで10半音）ので、ここも合わせる。
		// 間奏を持たない構成では、サブメロの盛り上がる側を `solo` の音色で受け持つ
