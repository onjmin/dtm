 ...
 * ## 組み立ての順番
 *
 * ①セクションの設計図を作る →②コード進行を割り当てる →③**各小節のリズム型を先に
 * 決める** →④その上に音を乗せる。音を先に置いて音価を均等割りすると、どうやっても
 * 「同じ長さの音が延々と続く」機械的な曲になる。
 *
 * 音の側は**2小節のモチーフ**を1つ作り、反復・セクエンツ・オクターブ上げの3通りに
 * 変形して展開する。小楽節は【問い2小節】＋【答え2小節】で、答えは問いのリズムを
 * 受けて着地音だけを変える。
 *
 * ## 引いて、測って、選ぶ
 *
 ...
 * ## 測り直し
 *
 * 生成物と参考コーパスを同じ物差しで並べるスクリプトが `scripts/compare-*.ts` にある
 * （特徴量・小節ごとの密度・リズムの語彙・完全一致の反復・音高）。ここを触ったら
 ...
 *
 * ここまでの生成は音階の度数（{@link MAJOR_SCALE}）だけで組み立てるので、進行に
 * セカンダリドミナントを入れても旋律が付いていかない——`E7` の上で `G` を鳴らして
 * 和音の `G#` とぶつかるか、避けて `A` へ逃げるかしかなくなる。参考曲の非ダイアトニック
...
export type VocalPlan = {
	/**
	 * 2人目が歌う区間 `[開始ステップ, 終了ステップ)`。**小節番号ではなく区間で持つ**
	 * ——小節線でぴったり交代すると受け渡しが機械的に聞こえるので、次の人が
	 * 手前から食い気味に入れるようにしてある。
	 */
...
const draw = (
	...
	// 「同じフレーズが返ってきた」という手応えがどこにも生まれない。
	//
	// 各セクションは【問い2小節】＋【答え2小節】の小楽節を1〜2個持つ。8小節の
	// セクション（Aメロ・サビ）は「問い→答え→問いの変形→答え」、4小節（Bメロ等）は
	// 「問い→答え」。答えは問いのリズムを受けて着地音だけを変える。
